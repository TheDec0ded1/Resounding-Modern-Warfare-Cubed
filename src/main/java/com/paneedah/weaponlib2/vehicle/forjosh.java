...
public class forjosh {
	...
	public static ItemStackHandler getVMWSlots(EntityPlayer player)
    {
        Method m = null;
        try {
            m = ReflectionHelper.findMethod(Class.forName("com.paneedah.weaponlib2.compatibility.CompatibleCustomPlayerInventoryCapability"), "getInventory", null, EntityLivingBase.class);
        ...
        if(inventory != null){
            try {
                backpack = (ItemStack[]) ReflectionHelper.findField(Class.forName("com.paneedah.weaponlib2.inventory.CustomPlayerInventory"), "inventory").get(inventory);
            } catch (Exception e) { return null; }
        }

...
public class SpecialPatcher {
	...
	public static byte[] transform(final String obfuscated, final String deobfuscated, byte[] bytes) {
		...
		if (obfuscated.equals(starterThreadPath)) {
			...
			toInject.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "com/paneedah/weaponlib2/compatibility/CoreSoundInterceptor", "init",
		...
		if (obfuscated.equals("paulscode.sound.libraries.SourceLWJGLOpenAL")) {
			...
			toInject.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "com/paneedah/weaponlib2/compatibility/CoreSoundInterceptor",
